...
Why use Mint 20.2 ? For some reason, Python 3.10 throws a runtime error of questionable applicability on the sample problem. All is fine with Python 3.9, but as a policy decision, Mint restricts the installed Python sub-versions to even numbers. This makes 20.2 the natural choice, since it ships with Python 3.8.10, the highest version number available in Mint which is less than 3.10.
...
The target directory that was the subject of the $PATH statement in the .bashrc file should contain the 14 .py files that are contained the the Fluids directory of the __vmc_pde__ code folder. Right click on this target directory, open a terminal, and start Jupyter notebook from there. Open a notebook file such as the one included from the caveman repository, and execute the cell containing the __vmc_pde__ test problem. All the output for the sample problem that was shown in Idle should become available, though in Jupyter the figures will be generated without pause.
